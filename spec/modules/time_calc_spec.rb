...
RSpec.describe TimeCalc do
  ...
  describe "#date_diff_in_days" do
    context "in comparison to internal ruby calculations" do
      ...
      it "accurately calculates the diff from today" do
        ...
        today_date      = Date.today
        func_date_dif   = time_calc_obj.date_diff_in_days(historical_date, today_date)
  ...
  describe "#date_diff_in_years" do
    ...
    context "with multiple dates" do
      ...
      it "can accept a string date" do
        string_date  = in_pst(5.years.ago).to_s
        string_date2 = 4.years.ago.to_s
        age_in_years = time_calc_obj.date_diff_in_years(string_date, string_date2)
        expect(age_in_years).to be(1)
      end
      it "can accept date_time object" do
        string_date  = in_pst(5.years.ago).to_datetime
  ...
  describe "#date_diff_in_months" do
    ...
    context "with multiple dates" do
      it "returns the difference between the two dates in months" do
        ...
        input_date2 = 4.years.ago.to_date
        diff_in_months    = time_calc_obj.date_diff_in_months(input_date, input_date2)
        expect(diff_in_months).to be(12)
      end
      it "can accept a string date" do
        string_date  = 5.months.ago.to_s
        string_date2 = 4.months.ago.to_s
        diff_in_months = time_calc_obj.date_diff_in_months(string_date, string_date2)
        expect(diff_in_months).to be(1)
      end
      it "can accept date_time object" do
      ...
      it "can accept time object" do
        ...
        diff_in_months = time_calc_obj.date_diff_in_months(time_object, time_object2)
  ...
  describe "#time_string_to_time_hash" do
    context "with only one input time type (week, month, year)" do
      ...
      string_times.each do |string_time_data|
        ...
        it "takes an input string of #{input_string} and returns #{output_hash}" do
          expect(time_calc_obj.time_string_to_time_hash(input_string)).to eq(output_hash)
        end
      end
    end
  ...
  describe "#date_diff_vs_string_time_diff" do
    context 'with one date' do
      describe 'it takes a date and evaluates if more time has passed than represented of the time diff' do
        it 'returns true if the date difference is larger than the string date diff' do
          input_date        = 3.months.ago
          string_time_diff  = "2 months + 4 days"
          expect(
            time_calc_obj.date_diff_vs_string_time_diff(past_date: input_date,
                                                        time_diff_string: string_time_diff)
          ).to eq(true)
        end
        it 'returns true if the date difference is exact to the string date diff' do
          input_date        = 2.months.ago
          string_time_diff  = "2 months"
          expect(
            time_calc_obj.date_diff_vs_string_time_diff(past_date: input_date,
                                                        time_diff_string: string_time_diff)
          ).to eq(true)
        end
        it 'returns false if the date difference is larger than the string date diff' do
          input_date        = 2.months.ago
          string_time_diff  = "2 months + 4 days"
          expect(
            time_calc_obj.date_diff_vs_string_time_diff(past_date: input_date,
    ...
    context 'with two dates' do
      it 'returns true if the date difference is larger than the string date diff' do
        past_date          = 5.months.ago
        future_date        = 2.months.ago
        string_time_diff  = "2 months + 4 days"
        expect(
          time_calc_obj.date_diff_vs_string_time_diff(past_date: past_date,
                                                      future_date: future_date,
                                                      time_diff_string: string_time_diff)
        ).to eq(true)
      end
      it 'returns true if the date difference is exact to the string date diff' do
        past_date          = 4.months.ago
        future_date        = 2.months.ago
        string_time_diff  = "2 months"
        expect(
          time_calc_obj.date_diff_vs_string_time_diff(past_date: past_date,
  ...
  describe "#detailed_date_diff" do
    context "with 2 year/month/week, returns string with multiple year/month/week" do
      ...
      #   detailed_date_diff = time_calc_obj.detailed_date_diff(input_date, input_date2)
  ...
  describe "#validate_day_diff" do
    it "takes an day_diff and required_days and returns boolean" do
      day_diff = 100
      required_days = 80
      expect(time_calc_obj.validate_day_diff(day_diff, required_days)).to be(true)
    end
    ...
    it "returns false when the day_diff is lower than required days" do
      day_diff = 60
      required_days = 80
      expect(time_calc_obj.validate_day_diff(day_diff, required_days)).to be(false)
    end
  end
  ...
  describe "#date_minus_time_period" do
    ...
    it "takes an input date and a collection of different time units" do
      ...
      expect(time_calc_obj.date_minus_time_period(
  ...
  describe "#validate_time_period_diff" do
    ...
    it "can take one date and a collection of different time units" do
      years, months, weeks, days = 0, 6, 0, 0
      past_date                  = Date.new(2015, 5, 15)
      expect(
        time_calc_obj.validate_time_period_diff(past_date,
                                           years: years,
                                           months: months,
                                           weeks: weeks,
                                           days: days)
    ...
    it "can take two dates and a collection of different time units" do
      ...
      expect(
        time_calc_obj.validate_time_period_diff(
          past_date, future_date, years: years, months: months, weeks: weeks, days: days
        )
      ).to eq(true)
    end
       
    it "returns true when a date is exactly on the day differential" do
      years, months, weeks, days = 0, 1, 0, 0
      past_date                  = Date.new(2015, 1, 15)
