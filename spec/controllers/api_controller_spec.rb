...
RSpec.describe ApiController, type: :controller do
  ...
  describe 'checking access credentials' do
    [
      :import_patient_data,
      :import_vaccine_dose_data,
      :heartbeat
    ].each do |action|
      it "rejects POST #{action} without any credentials" do
        valid_payload = {}
        valid_payload[action] = 'no credentials'

        post action, valid_payload, format: :json
        expect(response.response_code).to eq(401)
        expect(response.body).to eq("HTTP Basic: Access denied.\n")
      end
      it "rejects POST #{action} with wrong credentials" do
        valid_payload = {}
        valid_payload[action] = 'wrong credentials'

        basic_auth 'not', 'correct'
        post action, valid_payload, format: :json
        expect(response.response_code).to eq(401)
        expect(response.body).to eq("HTTP Basic: Access denied.\n")
      end
